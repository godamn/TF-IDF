...
def get_corpus():
    ...
    path = "./input/bd_top3_random100000_sample.json"

    print("start reading corpus:", psutil.virtual_memory().percent)
    ...
    print("finish reading corpus:", psutil.virtual_memory().percent)
...
def process_corpus(corpus):
    # corpus_list = []
    corpus_list = manager.list([None for i in range(len(corpus))])

    num_process = 4
    pool = Pool(num_process)
    print("start multi-processing:", psutil.virtual_memory().percent)
    print("cutting corpus...")
    start = datetime.datetime.now()

    for i in range(num_process):
        pool.apply_async(process_thread, args=(i, num_process, corpus, corpus_list))

    print("finish multi-processing:", psutil.virtual_memory().percent)
    pool.close()
    print("finish multi-processing:", psutil.virtual_memory().percent)
    pool.join()
    print("finish multi-processing:", psutil.virtual_memory().percent)
...
def tf_idf():
    ...
    del contents
    print("start modeling:", psutil.virtual_memory().percent)
    ...
    print("finish modeling:", psutil.virtual_memory().percent)
...
def main():
    global manager
    filename = "compare_gensim_threads_100000"
